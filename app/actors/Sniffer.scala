...
class Sniffer(host: String, username: String, password: String, refreshTime: RefreshTime, freq: Option[FiniteDuration] = Some(5 minutes)) extends Actor with ActorLogging {
  ...
  override def preStart: Unit = {
    ...
    val gcalm = context.actorOf(Props(new GCalManager(refreshTime)), s"gcalmanager-$username-${Random.nextInt()}")
    this.gcalman = Some(gcalm)
  }

  var gcalman: Option[ActorRef] = None
  ...
  def idle(folder: IMAPFolder): Receive = {
    case Mails(msgs) =>
      msgs.foreach(msg => {
        gcalman.map(_ ! GCalManager.CreateEvent(msg))
      })
