          ...
          id => {
              DBUtils.getUser(state.toLong).map {
                user =>  {
                  ...
                  Global.snifferManager ! SnifferManager.StartSniffer((user, refreshTime.copy(id = user.id)))
    ...
    WS.client.url(Urls.TokenEndpoint)
      ...
      response => {
        ...
        val refreshTime = RefreshTime((tokens \ "access_token").asOpt[String].get, refreshToken,
          new Timestamp(new Date().getTime), (tokens \ "expires_in").asOpt[Long].get, state.toLong, None)
            ...
            if (folder.exists() && folder.isOpen) {
              DBUtils.fetchUser(imapCredentials.email).flatMap {
                optionUser => optionUser match {
                    ...
                    DBUtils.refreshTime(user.email).flatMap {
                      optionRefreshTime => optionRefreshTime match {
                        case Some(refreshTime) => {
                          ...
                          Future(Redirect(routes.Application.oauth2(user.id.get)))
                  ...
                  case None => {
                    DBUtils.createUser(User(imapCredentials.host, imapCredentials.email, imapCredentials.password)).flatMap {
                      id => {
                        if (id > 0) {
                          DBUtils.fetchUser(imapCredentials.email).map {
                            userOption => userOption match {
                                case Some(user) => {
                                  Redirect(routes.Application.oauth2(user.id.get))
                                }
                                case None => {
                          ...
                          Future(Redirect(routes.Application.home()).flashing("failure" -> "Couldn't create new user, try again"))
                        }
                      }
                    }.recover { case th => {
                      th.printStackTrace()
