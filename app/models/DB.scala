...
object DB {
  ...
  lazy val username = "admin6yqz7hc"
  lazy val password = "k7t7QADJUJpe"

  lazy val db = Database.forURL(
    url = "jdbc:postgresql://$OPENSHIFT_POSTGRESQL_DB_HOST:$OPENSHIFT_POSTGRESQL_DB_PORT/add2cal",
    driver = "com.postgressql.Driver",
    user = username,
    password = password
  )
